...
const VendorRegister = () => {
    ...
    const captureImage = useCallback(() => {
        setIsScanning(true); // Show scanning effect
        setTimeout(() => {
            const imageSrc = webcamRef.current.getScreenshot();
            setImage(imageSrc);
            setIsScanned(true);
            setIsScanning(false);
        }, 2000); // Delay to mimic scanning
    }, [webcamRef]);
    ...
    const handleSubmit = async (e) => {
        ...
        const imageFile = new File([blob], `vendor_${name.replace(/\s+/g, '_')}_${Date.now()}.png`, { type: "image/png" });
        ...
        try {
            const response = await fetch("http://127.0.0.1:8000/register_vendor/", {
                method: "POST",
                body: formData,
            });
